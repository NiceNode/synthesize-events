import {
	...
	nodePrefix,
	makeAKeyByDay,
	dailyActiveNodesSetByDay,
	dailyActiveNodesByDay,
	weeklyActiveNodesByDay,
	monthlyActiveNodesByDay,
	weeklyActiveNodesSetByDay,
	monthlyActiveNodesSetByDay,
	nodeServicePrefix,
} from "./redisTypes";
...
console.log(
	"Active nodes sets indexing threshold timestamps (daily, weekly, monthly): ",
	dailyThresholdTimeMs,
	weeklyThresholdTimeMs,
	monthlyThresholdTimeMs,
);
...
export const initializeDateConstants = (
	dayBeforeDayToIndexYyyyMmDd: string,
	dayToIndexDate: Date,
	argDayToIndexYyyyMmDd: string,
): void => {
	dayToIndexYyyyMmDd = argDayToIndexYyyyMmDd;
	// Separate thresholds for when the indexing day is different from today (yesterday?)
	indexDayStartOfDay = new Date(
		dayToIndexDate.getFullYear(),
		dayToIndexDate.getMonth(),
		dayToIndexDate.getDate(),
	);
	 ...
	 * Used to keep track of historically active nodes by day, week, and month
	 */
	dailyActiveNodesByDayKey = makeAKeyByDay(
		dailyActiveNodesSetByDay,
		dayToIndexYyyyMmDd,
	);
	weeklyActiveNodesByDayKey = makeAKeyByDay(
		weeklyActiveNodesSetByDay,
		dayToIndexYyyyMmDd,
	);
	monthlyActiveNodesByDayKey = makeAKeyByDay(
		monthlyActiveNodesSetByDay,
		dayToIndexYyyyMmDd,
	);
	dayBeforeWeeklyActiveNodesByDayKey = makeAKeyByDay(
		weeklyActiveNodesSetByDay,
		dayBeforeDayToIndexYyyyMmDd,
	);
	dayBeforeMonthlyActiveNodesByDayKey = makeAKeyByDay(
		monthlyActiveNodesSetByDay,
		dayBeforeDayToIndexYyyyMmDd,
	);
};
 ...
 * @param nodeJson
 */
export const indexSingleNodeReport = async (
	nodeJson: NodeJson,
): Promise<void> => {
	console.log("indexSingleNodeReport nodeId: ", nodeJson.nodeId);
	if (
		dailyThresholdTimeMs === null ||
		weeklyThresholdTimeMs === null ||
		monthlyThresholdTimeMs === null ||
		dailyActiveNodesByDayKey === null ||
		weeklyActiveNodesByDayKey === null ||
		monthlyActiveNodesByDayKey === null
	) {
	...
	if (nodeJson.lastRunningTimestampMs >= dailyThresholdTimeMs) {
		await impactDashRedisClient.addToSet(
			dailyActiveNodesByDayKey,
			nodeJson.nodeId,
		);
		console.log("Daily active node, nodeId: ", nodeJson.nodeId);
	}
	if (nodeJson.lastRunningTimestampMs >= weeklyThresholdTimeMs) {
		await impactDashRedisClient.addToSet(
			weeklyActiveNodesByDayKey,
			nodeJson.nodeId,
		);
		console.log("Weekly active node, nodeId: ", nodeJson.nodeId);
	}
	if (nodeJson.lastRunningTimestampMs >= monthlyThresholdTimeMs) {
		await impactDashRedisClient.addToSet(
			monthlyActiveNodesByDayKey,
			nodeJson.nodeId,
		);
		console.log("Monthly active node, nodeId: ", nodeJson.nodeId);
	}
};
...
const processNode = (
	...
	return async (nodeIdStrOrNum: string | number): Promise<void> => {
		...
		const node: NodeJson = await impactDashRedisClient.client.json.get(
			`${nodePrefix}${nodeId}`,
		);
		...
		if (timePeriod === "daily") {
			if (
				node.lastRunningTimestampMs == null ||
				node.lastRunningTimestampMs < dailyThresholdTimeMs
			) {
			...
			if (
				node.lastRunningTimestampMs == null ||
				node.lastRunningTimestampMs < weeklyThresholdTimeMs
			) {
			...
			if (
				node.lastRunningTimestampMs == null ||
				node.lastRunningTimestampMs < monthlyThresholdTimeMs
			) {
				console.warn(
					`Removing ${nodeId} from monthly set...${node.lastRunningTimestampMs} and ${monthlyThresholdTimeMs}`,
				);
				await impactDashRedisClient.removeFromSet(setKey, nodeId);
			} else {
		...
		if (indexToUpdate !== undefined) {
			// console.log(
			// 	`Indexing ${nodeId} ${JSON.stringify(node)} to ${JSON.stringify(indexToUpdate)} set`,
			// );
			...
			if (node.country) {
				if (
					indexToUpdate.specId[node.specId].country[node.country] === undefined
				) {
					indexToUpdate.specId[node.specId].country[node.country] = 0;
				}
			...
			if (node.network) {
				if (
					indexToUpdate.specId[node.specId].networks[node.network] === undefined
				) {
					indexToUpdate.specId[node.specId].networks[node.network] = 0;
				}
			...
			for (let i = 0; i < node.serviceIds.length; i++) {
				...
				const service = await impactDashRedisClient.client.json.get(
					`${nodeServicePrefix}${serviceId}`,
				);
				if (service === null) {
					throw new Error(
						`Indexing ${nodeId} service ${serviceId} not found in redis`,
					);
				}
				const serviceSpecId = service.specId;
				if (servicesIndex[serviceSpecId] === undefined) {
					servicesIndex[serviceSpecId] = {
						count: 0,
						country: {},
						networks: {},
					};
				...
				if (service.country) {
					if (
						servicesIndex[serviceSpecId].country[service.country] === undefined
					) {
						servicesIndex[serviceSpecId].country[service.country] = 0;
					}
				...
				if (service.network) {
					if (
						servicesIndex[serviceSpecId].networks[service.network] === undefined
					) {
						servicesIndex[serviceSpecId].networks[service.network] = 0;
					}
...
export const copyPreviousWeekMonthActiveNodes = async (): Promise<void> => {
	if (
		...
		weeklyActiveNodesByDayKey === null ||
		monthlyActiveNodesByDayKey === null ||
		dayToIndexYyyyMmDd === null ||
		dayBeforeWeeklyActiveNodesByDayKey === null ||
		dayBeforeMonthlyActiveNodesByDayKey === null
	) {
		throw new Error("A required time, date, or key is null.");
	}
	...
	console.log(
		`Redis copying set ${dayBeforeWeeklyActiveNodesByDayKey} to ${weeklyActiveNodesByDayKey}`,
	);
	const copyResult = await impactDashRedisClient.client.copy(
		dayBeforeWeeklyActiveNodesByDayKey,
		weeklyActiveNodesByDayKey,
		{ replace: true },
	);
	console.log("Copy result, ", copyResult);
	console.log(
		`Redis copying set ${dayBeforeMonthlyActiveNodesByDayKey} to ${monthlyActiveNodesByDayKey}`,
	);
	const copyResult2 = await impactDashRedisClient.client.copy(
		dayBeforeMonthlyActiveNodesByDayKey,
		monthlyActiveNodesByDayKey,
		{ replace: true },
	);
...
export const indexActiveNodeSets = async (): Promise<void> => {
	...
	await iterateSet(
		impactDashRedisClient,
		dailyActiveNodesByDayKey,
		processNode(dailyActiveNodesByDayKey, "daily"),
	);
	await iterateSet(
		impactDashRedisClient,
		weeklyActiveNodesByDayKey,
		processNode(weeklyActiveNodesByDayKey, "weekly"),
	);
	await iterateSet(
		impactDashRedisClient,
		monthlyActiveNodesByDayKey,
		processNode(monthlyActiveNodesByDayKey, "monthly"),
	);

	console.log("Done: dailyActiveNodesIndex: ", dailyActiveNodesIndex);
	console.log("Done: weeklyActiveNodesIndex: ", weeklyActiveNodesIndex);
	console.log("Done: monthlyActiveNodesIndex: ", monthlyActiveNodesIndex);

	// save indexed data
	await impactDashRedisClient.client.json.set(
		makeAKeyByDay(dailyActiveNodesByDay, dayToIndexYyyyMmDd),
		"$",
		dailyActiveNodesIndex,
	);
	await impactDashRedisClient.client.json.set(
		makeAKeyByDay(weeklyActiveNodesByDay, dayToIndexYyyyMmDd),
		"$",
		weeklyActiveNodesIndex,
	);
	await impactDashRedisClient.client.json.set(
		makeAKeyByDay(monthlyActiveNodesByDay, dayToIndexYyyyMmDd),
		"$",
		monthlyActiveNodesIndex,
	);
};
